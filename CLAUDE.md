...
- **Prerendered HTML for crawlers (`functions/_prerender.js`)** → the site is a client-rendered SPA, so the HTML it served carried 6–12 words, zero `<h1>` and one link on every URL — identical for Googlebot. `functions/_middleware.js` now injects static HTML into `#root` for every composed page, built from the same `page-compositions.json` the React `PageRenderer` reads (plus `faqs.json`, `deals.json`, `testimonials.json`, `bio.json`, `press.json` and the briefings index for the data-driven sections). React clears it on mount, so humans still get the styled page. Nav + footer ship as real anchors so authority flows between pages. `/admin`, `/partners` and `/intel` are never prerendered; draft briefings are excluded. Same no-JSON-imports contract as `_ai-guide.js` and `_meta.js` so `tests/prerender.test.js` can import it — that test is the SEO contract (one `<h1>` per page, headings in order, no leaked markdown, escaped data). **Still not prerendered**: `/privacy` and `/terms` (content is hardcoded in `src/pages/Legal.jsx`, not a data file) and `/team` (no entry in `page-compositions.json` at all — the page renders empty in React too).
- **Case briefings (`case-briefing` section type)** → the reusable long-form template for a single case: dated status block, case-facts table, Markdown body sections (pipe tables supported), update log, FAQs and sources. Built to be *citable* — journalists, law firms and LLMs link to reference material, not landing pages — so **the selling CTA is off by default** and is one checkbox (`ctaEnabled`) in the section editor. Structure enforces "background is fixed, status is dated": keep every figure that goes stale in the status block, stamp it with `statusAsOf`, and record changes in the update log, so the page is refreshed rather than rewritten. Files: `src/components/sections/CaseBriefingSection.jsx`, the `case-briefing` entry in `section-types.json`, the editor block in `visualeditor/SectionEditorFields.jsx` (shares the generic `RowsEditor`), and the prerender in `functions/_prerender.js`. First instance: `/cases/terraform-labs` (`status: draft`, deliberately absent from `nav.json`). **Page Builder status now has teeth**: a composition whose `status` is not `active` is excluded from `sitemap.xml` and served `X-Robots-Tag: noindex`, so a draft is reviewable at its URL without being advertised. Flip it to Active in `/admin/page-builder` to launch.
- **Citation policy for case pages (`src/data/citation-policy.json`)** → a `case-briefing` page may cite ONLY court/docket records and official case-administration sites, government releases, the subject company's own disclosures, and reputable general/financial/legal media (WSJ, NYT, Bloomberg, Law360, Reuters, PR Newswire, Business Wire, AP). **No crypto trade blogs, no aggregators or case-tracker sites, and never a competitor** — linking one hands them the referral and the authority on our own page. An official company channel on a generic host (a plan-administrator notice on Medium) is primary, but is allowlisted by exact URL prefix under `companyDisclosureUrls`, never by host. `tests/prerender.test.js` enforces this across every source entry AND every inline body link, plus a named competitor blocklist. To add an outlet, edit the JSON — do not make a per-page exception.
