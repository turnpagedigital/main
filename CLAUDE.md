...
- **Add/manage a referral partner** → `src/data/referral-partners.json` (code, Attio record, portal-key hash). Full recipe incl. key generation + Attio schema notes: `docs/marketing/referral-partners.md`. Partner-facing portal lives at `/partners` (unlisted), API under `functions/api/partner/`.
