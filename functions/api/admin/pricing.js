 ...
 * GET  → { ok, data: { selfRecovery, publisherRecovery, payoutRatePct }, sha }
 * PUT  → body { selfRecovery, publisherRecovery, payoutRatePct }; commits via GitHub.
 *
...
export async function onRequestGet({ request, env }) {
  ...
  if (!r.ok) return jsonResponse({ ok: false, error: r.error || "GitHub read failed" }, 502);
...
export async function onRequestPut({ request, env }) {
  ...
  if (!current.ok) return jsonResponse({ ok: false, error: current.error || "GitHub read failed" }, 502);
  ...
  const result = await commitFileToGitHub(env, PRICING_PATH, newContent, current.sha, "Admin: update pricing");
