...
10. **DO NOT put the Watchlist in the right column or fold it into Storylines.** The Watchlist is a separate component in the *center column*, inside `advisory-body`, immediately under "Story of the Day". Storylines (right column) are filed / ripe matters organized by subject thread; the Watchlist is the pre-filing pipeline — leading indicators that have not yet ripened into a complaint, charge, filing, or order. Mixing the two collapses a useful distinction. Items also must not migrate back-and-forth between Watchlist and Storylines without an explicit ripening event (filing, charge, adjudication) that gets noted in the advisory body.
...
- Every per-topic dashboard contains exactly one `<section class="watchlist-box">`, positioned inside `advisory-body` (center column) under "Story of the Day". Right column remains Storylines only — the Watchlist never goes back into `col-right`.
...
2. **News fetch.** Run the tab's `news_fetch_command` if set (300s timeout). **Cowork-environment caveat:** the node Google-News scrapers (`scripts/fetch-news.cjs`) and `yfinance` cannot resolve DNS in the Cowork sandbox — they fail with `getaddrinfo EAI_AGAIN` and silently write stale data. Do NOT rely on them for the scan when running in Cowork. Instead, perform the news scan with the `WebSearch` tool (which has working egress here), running 2–4 focused queries per tab against the matters in the prior advisory plus the tab's `themes[]`, and use `mcp__workspace__web_fetch` / `WebFetch` for individual articles. The node scraper remains the scan path only in the GitHub Actions deploy pipeline (`daily-briefing-site/scripts/generate.py`), where outbound HTTPS is open; that script now performs its own Google-News-RSS scan filtered against `sources.md` before composing each advisory.
...
4. **Compose today's advisory** by layering today's NEW and DELTA content onto the prior advisory. Rebuild the lede each day to surface today's most consequential developments first. Below the lede, the body carries forward prior depth with today's deltas threaded in. The output must hit the 1,500–2,500 word floor at the Bartz-passage density level — never shrink the format to save effort. Save to `<project-dir>/public/advisory-YYYY-MM-DD.md`.
5. **Draft LinkedIn + X.com posts.** Save to `<project-dir>/posts/YYYY-MM-DD.md`. **Single-theme rule — non-negotiable.** Each LinkedIn post focuses on ONE matter, ONE legal question, ONE legislative issue, or ONE doctrinal thread — not a multi-case roundup. Acceptable post archetypes: (a) deep dive on a single case update (caption, docket, judge, parties, today's filing or order), (b) a focused legal question with the cited authority that frames it, (c) a legislative-track update on a specific bill, (d) a doctrinal pivot (one ruling and what it reorders). Forbidden: bullet roundups that touch 3+ unrelated cases; "today's mega-class action calibration — three deals are moving" leades; calendar-listing posts. The X.com companion is the same theme compressed to ≤280 chars. The post may glance at adjacent matters in a single closing line if context truly demands, but the body must read as one argument about one development. Posts lead with today's NEW item with the highest news-weight; never lead with a STALE matter. **Then also update `<project-dir>/posts.html` IN PLACE** — refresh the page-title `.stamp` to today's date, and replace the LinkedIn `<textarea id="linkedin-text">` and X.com `<textarea id="x-text">` body content with today's drafts. The `.md` file is the archive; `posts.html` is what renders to Andrew. If posts.html is left at yesterday's draft, the run is incomplete — every daily run must update both the `.md` archive and the rendered HTML.
...
7. **Refresh the pre-filing Watchlist.** Each tab carries a `watchlist:` block in its `tabs/NN-<slug>.md` config — a curated list of *pre-filing / pre-event* leading indicators (situations developing toward potential ripeness but not yet a filed case / charged action / adjudicated event). On every daily run:
   - **Carry forward** the prior items as a starting set; do not blank-slate them.
   - **Scan against the `watchlist.research_themes`** for new pre-filing names surfacing in the last 24-48 hours (advisor hires, withdrawal halts, short-seller reports, regulator inquiries, missed coupons, demand letters, cease-and-desist signaling — whatever the per-tab themes specify).
   - **Retire ripened items.** If a prior watchlist name **became ripe** since the last run (filed Ch. 11, was charged, complaint filed, order issued), remove it from `watchlist.items` and note the transition in today's advisory body (`X moved from Watchlist to Storylines this week — see [matter].`).
   - **Add at most 1-2 net-new items per run.** Watchlists should churn slowly; they're leading indicators, not daily news. Aim for a steady 3-5 items per tab.
   - **Update the tab config in place.** Edit `tabs/NN-<slug>.md` `watchlist:` YAML block — keep entries terse: `name` (bolded display string) + `signal` (one-line pre-filing signal, ≤ 150 chars).

## Step 3.5 — Inject Watchlist sections into per-tab dashboards

After all 6 per-tab dashboards are built and after every tab's `watchlist:` block has been refreshed per Step 3.7, run:

```bash
python3 /Users/waquoitcapital/Library/CloudStorage/Dropbox/Career/Current\ Roles/Turnpage/Development/daily-briefing/scripts/apply_watchlist.py
```

This reads each tab's `watchlist.items` from `tabs/NN-<slug>.md`, renders a compact `<section class="watchlist-box">` (paper-style, lighter weight than Storylines), and injects it inside `advisory-body` in the center column — **immediately under "Story of the Day", before the close of the article**. The script is idempotent: it strips any prior watchlist-box wherever it lives in the dashboard before re-inserting in the canonical location, so re-runs are safe.

If a tab has no `watchlist:` block (or an empty `items:`), the script skips that tab and prints a notice. The dashboard renders without a Watchlist section in that case. Failures don't abort the daily run.

## Step 3.6 — Fetch live ticker data + inject Market Watch widgets
...
2. **Rich format is non-negotiable.** Hard floor: 1,500 words per topic, 2,500-word ceiling on heavy news days. Density at the Bartz-passage level (full case caption, docket number, judge name, courtroom, dollar figures, percentages, statutory citations) throughout.
...
9. **Pre-filing Watchlist on every dashboard.** Every per-topic dashboard must carry a Watchlist section under "Story of the Day" in the center column, sourced from the tab config's `watchlist.items` block. Items are *pre-filing / pre-event* leading indicators only — never duplicate Storylines content (Storylines = already-filed / ripe matters; Watchlist = developing-but-not-yet-ripe). The Watchlist is curated, not regenerated daily — most items persist across runs; net-new additions should be 1-2 per run, with ripened items retired into the advisory body and removed from `watchlist.items`.
