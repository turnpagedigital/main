...
### Watchlist (center column, under Story of the Day)

The Watchlist is a compact box rendered **inside `advisory-body` in the center column**, immediately under the last paragraph of "Story of the Day" and before the close of the article. It is a separate component from the right-column Storylines and serves a different purpose: Storylines = already-filed / ripe matters organized by subject thread; Watchlist = **pre-filing / pre-event leading indicators** — situations developing toward potential ripeness but not yet a filed case, charged action, or adjudicated event.

Items are sourced from each tab's `watchlist.items` block in `tabs/NN-<slug>.md` and rendered by `scripts/apply_watchlist.py`. Aim for 3–5 items per tab; items persist across runs (the Watchlist churns slowly, since it tracks pipeline rather than daily news). When a Watchlist item ripens (filing, charge, adjudication), it gets removed from `watchlist.items` and noted in that day's advisory body.

```html
<section class="watchlist-box">
  <h2>Watchlist</h2>
  <p class="watchlist-sub">Pre-filing · developing · not yet ripe</p>
  <ul class="watchlist-list">
    <li class="watchlist-item">
      <span class="watchlist-name">Example Co.</span>
      <span class="watchlist-signal">One-line pre-filing signal (advisor hire, stock drop, regulator inquiry).</span>
    </li>
    <!-- 3-5 items total -->
  </ul>
</section>
...
**Styling tokens (injected once per dashboard by `apply_watchlist.py`):**
...
**Content rules.** Each `name` is the bolded display string for the situation (company, sector cluster, agency action, etc.). Each `signal` is one terse line (≤ 150 chars) describing the pre-filing indicator and explicitly noting the absence of a filing / charge / order (e.g., "no Ch. 11 yet", "no complaint filed", "pre-enforcement", "pre-publication"). The Watchlist is intentionally less dense than Storylines — no SVG icons, no embedded doc chips, no accordion. Lower visual weight signals to the reader that these are leading indicators, not the day's news.

**Anti-patterns.** Do not duplicate Storyline items into the Watchlist. Do not regenerate items from scratch every day. Do not place the Watchlist in `col-right` (right column). Do not let the Watchlist grow past 5 items (it is curated, not aggregated).
